...
import { Search, Loader2, Award, BookOpen, AlertCircle, GraduationCap, Printer, Medal, CheckCircle2 } from 'lucide-react';
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (studentData) {
        ...
        setError('No student found with this roll number. Please check your Roll Number card.');
      }
    } catch (err) {
      console.error(err);
        ...
        <div className="space-y-8 animate-in fade-in zoom-in-95 duration-500">
          ...
          <div className="grid gap-10">
            {exams.length > 0 ? exams.map(exam => {
              ...
              return (
                <Card key={exam.id} className="overflow-hidden border-slate-200 shadow-xl rounded-[2rem] hover:border-[#1e40af]/30 transition-all group">
                  ...
                  <CardContent className="p-0">
                    <Table>
                      ...
                      <TableBody>
                        ...
                        <TableRow className="bg-slate-900 text-white font-bold h-20">
                          <TableCell className="pl-10 text-slate-400 font-black uppercase tracking-widest">Aggregate Results</TableCell>
                          <TableCell className="text-center text-3xl font-black text-white">{stats?.obtained}</TableCell>
              ...
              <Card className="border-dashed border-4 border-slate-100 bg-slate-50/30 rounded-[2.5rem]">
